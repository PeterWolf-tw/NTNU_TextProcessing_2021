#!/usr/bin/env python3
# -*- coding:utf-8 -*-


def cCommandCoRefResolver(inputLIST, coRefKeySTR, personSTR):

    if coRefKeySTR not in inputLIST:
        raise ValueError

    if personSTR not in inputLIST:
        raise ValueError

    personSTRIndex = inputLIST.index(personSTR)
    if inputLIST[personSTRIndex+1] == "的":
        return False
    elif inputLIST[personSTRIndex+1] == "之":
        return False

    coRefKeyIndex = inputLIST.index(coRefKeySTR)
    if coRefKeyIndex > personSTRIndex:
        return True
    else:
        return False


def coRefResolver(inputLIST, coRefKeySTR, personSTR):
    if coRefKeySTR not in inputLIST:
        raise ValueError

    if personSTR not in inputLIST:
        raise ValueError

    personSTRIndex = inputLIST.index(personSTR)
    coRefKeyIndex = inputLIST.index(coRefKeySTR)
    if coRefKeyIndex > personSTRIndex:
        return True
    else:
        return False


if __name__ == "__main__":

    inputSTR = "大雄聽胖虎的妹妹說靜香愛的是自己"
    inputLIST = ["大雄", "聽", "胖虎", "的", "妹妹", "說", "靜香", "愛", "的", "是", "自己"]
    coRefDICT = {inputLIST[10]: []}

    resultBOOL = cCommandCoRefResolver(inputLIST, inputLIST[10], inputLIST[0])
    if resultBOOL == True:
        coRefDICT[inputLIST[10]].append(inputLIST[0])
    else:
        pass

    resultBOOL = cCommandCoRefResolver(inputLIST, inputLIST[10], inputLIST[2])
    if resultBOOL == True:
        coRefDICT[inputLIST[10]].append(inputLIST[2])
    else:
        pass

    resultBOOL = cCommandCoRefResolver(inputLIST, inputLIST[10], inputLIST[4])
    if resultBOOL == True:
        coRefDICT[inputLIST[10]].append(inputLIST[4])
    else:
        pass

    resultBOOL = cCommandCoRefResolver(inputLIST, inputLIST[10], inputLIST[6])
    if resultBOOL == True:
        coRefDICT[inputLIST[10]].append(inputLIST[6])
    else:
        pass

    print(coRefDICT)
    
#-------------------------------------------------------------------------------------------

    inputSTR_N = "小夫告訴大雄其實靜香喜歡的是他"
    inputLIST_C = ["小夫", "告訴", "大雄", "其實", "靜香", "喜歡", "的", "是", "他"]
    coRefDICT_T = {inputLIST_C[8]: []}

    #小夫0，大雄2，靜香4；他可以是小夫也可以是大雄
    
    #他=小夫
    resultBOOL = cCommandCoRefResolver(inputLIST_C, inputLIST_C[8], inputLIST_C[0])
    if resultBOOL == True:
        coRefDICT_T[inputLIST_C[8]].append(inputLIST_C[0])
    else:
        pass

    #他=大雄
    resultBOOL = cCommandCoRefResolver(inputLIST_C, inputLIST_C[8], inputLIST_C[2])
    if resultBOOL == True:
        coRefDICT_T[inputLIST_C[8]].append(inputLIST_C[2])
    else:
        pass

    print(coRefDICT_T)

#-------------------------------------------------------------------------------------------
    
    inputSTR_1 = "大雄知道靜香喜歡的是自己"
    inputLIST_1 = ["大雄", "知道", "靜香", "喜歡", "的", "是", "自己"]
    coRefDICT_1 = {inputLIST_1[6]: []}

    #大雄0，靜香2；自己可對應至任一

    #自己=大雄
    resultBOOL = cCommandCoRefResolver(inputLIST_1, inputLIST_1[6], inputLIST_1[0])
    if resultBOOL == True:
        coRefDICT_1[inputLIST_1[6]].append(inputLIST_1[0])
    else:
        pass   

    #自己=靜香
    resultBOOL = cCommandCoRefResolver(inputLIST_1, inputLIST_1[6], inputLIST_1[2])
    if resultBOOL == True:
        coRefDICT_1[inputLIST_1[6]].append(inputLIST_1[2])
    else:
        pass     
    
    print(coRefDICT_1)

#-------------------------------------------------------------------------------------------

    inputSTR_2 = "大雄聽胖虎說靜香愛的是自己"
    inputLIST_2 = ["大雄", "聽", "胖虎", "說", "靜香", "愛", "的", "是", "自己"]
    coRefDICT_2 = {inputLIST_2[8]: []}

    #大雄0，胖虎2，靜香4；自己可對應至任一

    #自己=大雄
    resultBOOL = cCommandCoRefResolver(inputLIST_2, inputLIST_2[8], inputLIST_2[0])
    if resultBOOL == True:
        coRefDICT_2[inputLIST_2[8]].append(inputLIST_2[0])
    else:
        pass   

    #自己=胖虎
    resultBOOL = cCommandCoRefResolver(inputLIST_2, inputLIST_2[8], inputLIST_2[2])
    if resultBOOL == True:
        coRefDICT_2[inputLIST_2[8]].append(inputLIST_2[2])
    else:
        pass

    #自己=靜香
    resultBOOL = cCommandCoRefResolver(inputLIST_2, inputLIST_2[8], inputLIST_2[4])
    if resultBOOL == True:
        coRefDICT_2[inputLIST_2[8]].append(inputLIST_2[4])
    else:
        pass

    print(coRefDICT_2)
    
