...
def main(inputLIST,Unknown_X_LIST,X_LIST):
    coRefDICT = {}
    for X in X_LIST:
        coRefDICT[inputLIST[X]] = []
    for i in range(0, len(X_LIST)):
        for j in range(0, len(Unknown_X_LIST)):
            resultBOOL = cCommandCoRefResolver(inputLIST, inputLIST[X_LIST[i]], inputLIST[Unknown_X_LIST[j]])
            if resultBOOL == True:
                coRefDICT[inputLIST[X_LIST[i]]].append(inputLIST[Unknown_X_LIST[j]])
...
if __name__ == "__main__":
    ...
    Unknown_X_LIST = [0,2,4,6]
    X_LIST = [10]
    main(inputLIST,Unknown_X_LIST,X_LIST)
    ...
    Unknown_X_LIST = [0,2,4]
    X_LIST = [8]
    main(inputLIST,Unknown_X_LIST,X_LIST)

    #inputSTR = "大雄聽胖虎的妹妹說靜香愛的是自己" ans: {'自己': ['大雄', '妹妹', '靜香']}
